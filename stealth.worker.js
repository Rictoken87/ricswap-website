...
function numToSizedHex(num, size2) {
  return bytesToHex3(numberToBytesBE2(num, size2));
}
var _0n8 = BigInt(0);
var _1n8 = BigInt(1);
var _2n5 = BigInt(2);
...
function weierstrassPoints(opts) {
  ...
  function isWithinCurveOrder(num) {
    return inRange2(num, _1n8, CURVE.n);
  }
  function normPrivateKeyToScalar(key) {
    ...
    if (wrapPrivateKey)
      num = mod2(num, N2);
    aInRange2("private key", num, _1n8, N2);
    return num;
  }
  function aprjpoint(other) {
    if (!(other instanceof Point))
  ...
  class Point {
    ...
    multiply(scalar) {
      const { endo: endo2, n: N2 } = CURVE;
      aInRange2("scalar", scalar, _1n8, N2);
      let point, fake;
      if (endo2) {
...
function weierstrass2(curveDef) {
  ...
  function int2octets(num) {
    aInRange2("num < 2^" + nBitLength, num, _0n8, ORDER_MASK);
...
var STEALTH_CHAIN_SHORT_NAME = "drive";
...
function getPoint(pub) {
  try {
    return secp256k1.ProjectivePoint.fromHex(bytesToHex(pub));
  } catch (err) {
    throw new Error(`invalid secp256k1 public key: ${err.message}`);
  }
}
function ecdhUncompressed(privateKey, publicKey) {
  const priv = requireValidScalar(bytesToBigInt(privateKey), "ECDH private key");
  if (typeof secp256k1.getSharedSecret === "function") {
    const shared = secp256k1.getSharedSecret(privateKey, publicKey, false);
...
function hashSharedSecret(sharedUncompressed) {
  ...
  const hash = keccak_256(sharedUncompressed);
  const scalar = requireValidScalar(bytesToBigInt(hash), "hashed shared secret");
  return { hash, scalar, viewTag: hash[0] };
}
...
function normalizeAddress(addr) {
  const raw = strip0x(addr);
  if (raw.length !== 40 || !HEX_RE.test(raw)) {
    throw new Error("invalid 20-byte address");
  }
  return toChecksumAddress(fromHex(raw));
}
...
function parsePrivateKey(input) {
  const bytes = typeof input === "string" ? fromHex(input) : Uint8Array.from(input);
  if (bytes.length !== 32) throw new Error("private key must be 32 bytes");
  requireValidScalar(bytesToBigInt(bytes), "private key");
  return bytes;
}
...
function keyPairFromPrivate(privateKey) {
  ...
  return { privateKey: toHex2(privateKey), publicKey: toHex2(publicKey) };
}
function encodeStealthMetaAddress(spendPublicKey, viewPublicKey, shortName = STEALTH_CHAIN_SHORT_NAME) {
  const spend = compressPub(parsePubKey(spendPublicKey));
  const view = compressPub(parsePubKey(viewPublicKey));
  const concat2 = new Uint8Array(META_COMPRESSED_LEN);
  concat2.set(spend, 0);
  concat2.set(view, 33);
  return `st:${shortName}:${toHex2(concat2)}`;
}
function readViewTag(metadata) {
  const bytes = typeof metadata === "string" ? fromHex(metadata) : metadata;
...
function scalarFromKeyMaterial(material) {
  let digest = keccak_256(material);
  for (let i = 0n; i < 64n; i++) {
    const s = modN(bytesToBigInt(digest));
    if (isValidScalar(s)) return bigIntToBytes32(s);
    digest = keccak_256(concatBytes4(digest, bigIntToBytes32(i + 1n)));
  }
  throw new Error("failed to derive a valid secp256k1 scalar");
}
function generateStealthKeyPairFromSignature(signature) {
  const sig = fromHex(signature);
  if (sig.length < 65) throw new Error("signature must be at least 65 bytes");
  const seed = keccak_256(sig);
  const spendPriv = scalarFromKeyMaterial(
    concatBytes4(seed, new TextEncoder().encode("spend-v1"))
  );
  const viewPriv = scalarFromKeyMaterial(
    concatBytes4(seed, new TextEncoder().encode("view-v1"))
  );
  try {
    const spend = keyPairFromPrivate(spendPriv);
    const view = keyPairFromPrivate(viewPriv);
    return {
      spend,
      view,
      stealthMetaAddress: encodeStealthMetaAddress(spend.publicKey, view.publicKey),
...
function extractLogField(log, key) {
  if (log.args && log.args[key] !== void 0 && log.args[key] !== null) {
    return log.args[key];
  }
  return log[key];
}
function asBytes(value, label) {
  ...
  throw new Error(`${label} missing from announcement log`);
}
function asSchemeId(value) {
  if (value === void 0 || value === null) return SCHEME_ID_SECP256K1;
  ...
  if (typeof value === "number") {
    if (!Number.isInteger(value) || value < 0) {
      throw new Error("schemeId is not a non-negative integer");
    }
    return BigInt(value);
  }
  throw new Error("schemeId has unsupported type");
}
function checkAnnouncement(eventLog, userViewPrivateKey, userSpendPublicKey) {
  const schemeId = asSchemeId(extractLogField(eventLog, "schemeId"));
  const announcedAddress = normalizeAddress(String(extractLogField(eventLog, "stealthAddress") ?? ""));
  const ephemeralPub = parsePubKey(asBytes(extractLogField(eventLog, "ephemeralPubKey"), "ephemeralPubKey"));
  const metadataField = extractLogField(eventLog, "metadata");
  const metadata = metadataField === void 0 || metadataField === null ? new Uint8Array(0) : asBytes(metadataField, "metadata");
  const viewPriv = parsePrivateKey(userViewPrivateKey);
  const spendPub = parsePubKey(userSpendPublicKey);
  try {
    const shared = ecdhUncompressed(viewPriv, ephemeralPub);
    const { scalar, viewTag } = hashSharedSecret(shared);
    const announcedTag = readViewTag(metadata);
    const viewTagMatches = announcedTag === null ? true : announcedTag === viewTag;
    if (!viewTagMatches) {
      return { isForUser: false, stealthAddress: null, viewTagMatches: false, schemeId };
    }
    const stealthPoint = getPoint(spendPub).add(secp256k1.ProjectivePoint.BASE.multiply(scalar));
    const derived = pubkeyToAddress(stealthPoint.toRawBytes(false));
    const isForUser = derived.toLowerCase() === announcedAddress.toLowerCase();
    return {
      isForUser,
      stealthAddress: isForUser ? derived : null,
      viewTagMatches: true,
      schemeId
    };
  } finally {
...
function computeSpendingPrivateKey(ephemeralPubKey, userViewPrivateKey, userSpendPrivateKey) {
  const ephPub = parsePubKey(ephemeralPubKey);
  const viewPriv = parsePrivateKey(userViewPrivateKey);
  const spendPriv = parsePrivateKey(userSpendPrivateKey);
  try {
    const shared = ecdhUncompressed(viewPriv, ephPub);
    const { scalar } = hashSharedSecret(shared);
    const stealthScalar = requireValidScalar(bytesToBigInt(spendPriv) + scalar, "stealth private key");
    const stealthPrivBytes = bigIntToBytes32(stealthScalar);
    const stealthPub = secp256k1.getPublicKey(stealthPrivBytes, false);
    const stealthAddress = pubkeyToAddress(stealthPub);
    const stealthPrivateKey = toHex2(stealthPrivBytes);
    zeroize(stealthPrivBytes);
...
// frontend/keyring.ts
function logEphemeral(log) {
  const raw = log.args?.ephemeralPubKey ?? log.ephemeralPubKey;
  if (typeof raw === "string") return raw;
  throw new Error("announcement missing ephemeralPubKey");
}
var IsolatedKeyring = class {
  #spend;
  #view;
  identity = null;
  get locked() {
    return this.#spend === void 0 || this.#view === void 0;
  }
  unlockFromSignature(signature) {
    const kp = generateStealthKeyPairFromSignature(signature);
    this.#spend = kp.spend.privateKey;
    this.#view = kp.view.privateKey;
    this.identity = {
      stealthMetaAddress: kp.stealthMetaAddress,
      spendPublicKey: kp.spend.publicKey,
      viewPublicKey: kp.view.publicKey
    };
  ...
  scan(logs) {
    if (!this.#view || !this.identity) throw new Error("keyring is locked");
    const hits = [];
    for (const log of logs) {
      const match = checkAnnouncement(log, this.#view, this.identity.spendPublicKey);
      if (!match.isForUser || !match.stealthAddress) continue;
      hits.push({
        stealthAddress: match.stealthAddress,
        ephemeralPubKey: logEphemeral(log)
      });
    }
    return hits;
  }
  async signSweep(input) {
    if (!this.#spend || !this.#view) throw new Error("keyring is locked");
    if (input.chainId > BigInt(Number.MAX_SAFE_INTEGER)) {
      throw new Error("chainId exceeds signer numeric range");
    }
    const spending = computeSpendingPrivateKey(input.ephemeralPubKey, this.#view, this.#spend);
    if (spending.stealthAddress.toLowerCase() !== input.expectedStealthAddress.toLowerCase()) {
      throw new Error("derived stealth address does not match announcement");
    }
    const account = privateKeyToAccount(spending.stealthPrivateKey);
    ...
    return { rawTransaction, stealthAddress: spending.stealthAddress };
  }
};

// web/stealth.worker.ts
var ring = new IsolatedKeyring();
self.addEventListener("message", (ev) => {
  ...
  void (async () => {
    try {
      switch (msg.type) {
        case "unlock": {
          const identity = ring.unlockFromSignature(msg.signature);
          post({ id: msg.id, ok: true, type: "unlock", identity });
          return;
        }
        case "forget": {
          ring.forget();
          post({ id: msg.id, ok: true, type: "forget" });
          return;
        }
        case "status": {
          post({
        ...
        case "scan": {
          post({ id: msg.id, ok: true, type: "scan", hits: ring.scan(msg.logs) });
          return;
        }
        case "signSweep": {
          const out = await ring.signSweep(msg.input);
          post({ id: msg.id, ok: true, type: "signSweep", ...out });
          return;
        }
        default:
      ...
      const error = err instanceof Error ? err.message : "worker error";
