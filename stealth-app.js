...
function Field(ORDER, bitLen4, isLE2 = false, redef = {}) {
  if (ORDER <= _0n3)
...
var _0n7, _1n7, hasHexBuiltin3, hexes4, asciis3, isPosBig2, bitMask2, u8n2, u8fr2, validatorFns2;
...
function numToSizedHex2(num2, size5) {
  return bytesToHex4(numberToBytesBE2(num2, size5));
}
var _0n10 = BigInt(0);
var _1n10 = BigInt(1);
var _2n6 = BigInt(2);
...
function weierstrassPoints2(opts) {
  ...
  function normPrivateKeyToScalar(key) {
    ...
    if (wrapPrivateKey)
      num2 = mod2(num2, N2);
    aInRange2("private key", num2, _1n10, N2);
    return num2;
  }
  function aprjpoint(other) {
    if (!(other instanceof Point2))
  ...
  class Point2 {
    ...
    multiply(scalar) {
      const { endo: endo2, n: N2 } = CURVE;
      aInRange2("scalar", scalar, _1n10, N2);
      let point, fake;
      if (endo2) {
        const { k1neg, k1, k2neg, k2 } = endo2.splitScalar(scalar);
        let { p: k1p, f: f1p } = this.wNAF(k1);
...
var isPosBig3 = (n) => typeof n === "bigint" && _0n12 <= n;
...
function bitLen3(n) {
  let len;
  for (len = 0; n > _0n12; n >>= _1n12, len += 1)
    ;
  return len;
}
var bitMask3 = (n) => (_1n12 << BigInt(n)) - _1n12;
...
function sqrt3mod43(Fp, n) {
  const p1div4 = (Fp.ORDER + _1n13) / _4n5;
  const root = Fp.pow(n, p1div4);
...
function FpLegendre3(Fp, n) {
  const p1mod2 = (Fp.ORDER - _1n13) / _2n8;
  const powered = Fp.pow(n, p1mod2);
  const yes = Fp.eql(powered, Fp.ONE);
  const zero = Fp.eql(powered, Fp.ZERO);
  const no = Fp.eql(powered, Fp.neg(Fp.ONE));
...
function Field3(ORDER, bitLenOrOpts, isLE2 = false, opts = {}) {
  ...
  const f = Object.freeze({
    ...
    isValid: (num2) => {
      if (typeof num2 !== "bigint")
        throw new Error("invalid field element: expected bigint, got " + typeof num2);
      return _0n13 <= num2 && num2 < ORDER;
    },
    is0: (num2) => num2 === _0n13,
    // is valid and invertible
    isValidNot0: (num2) => !f.is0(num2) && f.isValid(num2),
    isOdd: (num2) => (num2 & _1n13) === _1n13,
    ...
    fromBytes: (bytes, skipValidation = true) => {
      ...
      if (bytes.length !== BYTES)
        throw new Error("Field.fromBytes: expected " + BYTES + " bytes, got " + bytes.length);
      let scalar = isLE2 ? bytesToNumberLE3(bytes) : bytesToNumberBE3(bytes);
      if (modFromBytes)
...
function calcOffsets3(n, window2, wOpts) {
  const { windowSize, mask, maxNumber, shiftBy } = wOpts;
  let wbits = Number(n & mask);
  let nextN = n >> shiftBy;
  if (wbits > windowSize) {
...
var wNAF3 = class {
  ...
  wNAF(W, precomputes, n) {
    ...
    for (let window2 = 0; window2 < wo.windows; window2++) {
      const { nextN, offset, isZero, isNeg, isNegF, offsetF } = calcOffsets3(n, window2, wo);
      n = nextN;
      if (isZero) {
  ...
  wNAFUnsafe(W, precomputes, n, acc = this.ZERO) {
    ...
    for (let window2 = 0; window2 < wo.windows; window2++) {
      if (n === _0n14)
        break;
      const { nextN, offset, isZero, isNeg } = calcOffsets3(n, window2, wo);
      n = nextN;
      if (isZero) {
        continue;
      } else {
...
function createField(order, field, isLE2) {
  if (field) {
    ...
    return Field3(order, { isLE: isLE2 });
  }
}
...
// node_modules/@noble/curves/esm/abstract/weierstrass.js
var divNearest3 = (num2, den) => (num2 + (num2 >= 0 ? den : -den) / _2n9) / den;
function _splitEndoScalar(k, basis, n) {
  ...
  const k2neg = k2 < _0n15;
  if (k1neg)
    k1 = -k1;
  if (k2neg)
    k2 = -k2;
  const MAX_NUM = bitMask3(Math.ceil(bitLen3(n) / 2)) + _1n15;
  if (k1 < _0n15 || k1 >= MAX_NUM || k2 < _0n15 || k2 >= MAX_NUM) {
    throw new Error("splitScalar (endomorphism): failed, k=" + k);
  }
  return { k1neg, k1, k2neg, k2 };
}
...
function _normFnElement(Fn, key) {
  ...
  if (typeof key === "bigint") {
    ...
    let bytes = ensureBytes3("private key", key);
    try {
...
function weierstrassN(params, extraOpts = {}) {
  ...
  class Point2 {
    ...
    static fromHex(hex) {
      return Point2.fromBytes(ensureBytes3("pointHex", hex));
    }
    get x() {
      return this.toAffine().x;
    }
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  const lengths = Object.assign(getWLengths(Point2.Fp, Fn), { seed: getMinHashLength3(Fn.ORDER) });
  ...
  function randomSecretKey2(seed = randomBytes_(lengths.seed)) {
    return mapHashToField3(_abytes2(seed, lengths.seed, "seed"), Fn.ORDER);
  }
  ...
  function isProbPub(item) {
    ...
    if (Fn.allowedLengths || secretKey === publicKey)
      return void 0;
    const l = ensureBytes3("key", item).length;
  ...
  const utils = {
    ...
    randomPrivateKey: randomSecretKey2,
...
function ecdsa(Point2, hash3, ecdsaOpts = {}) {
  ...
  class Signature {
    ...
    static fromBytes(bytes, format2 = defaultSigOpts_format) {
      ...
      let recid;
      if (format2 === "der") {
        const { r: r2, s: s2 } = DER3.toSig(_abytes2(bytes));
    ...
    recoverPublicKey(messageHash) {
      ...
      const h = bits2int_modN(ensureBytes3("msgHash", messageHash));
    ...
    toBytes(format2 = defaultSigOpts_format) {
      validateSigFormat(format2);
      if (format2 === "der")
        return hexToBytes2(DER3.hexFromSig(this));
    ...
    static fromCompact(hex) {
      return Signature.fromBytes(ensureBytes3("sig", hex), "compact");
    }
    static fromDER(hex) {
      return Signature.fromBytes(ensureBytes3("sig", hex), "der");
    }
    normalizeS() {
  ...
  function int2octets(num2) {
    aInRange3("num < 2^" + fnBits, num2, _0n15, ORDER_MASK);
  ...
  function sign(message, secretKey, opts = {}) {
    message = ensureBytes3("message", message);
    const { seed, k2sig } = prepSig(message, secretKey, opts);
    const drbg = createHmacDrbg3(hash3.outputLen, Fn.BYTES, hmac2);
  ...
  function tryParsingSig(sg) {
    ...
    if (isObj) {
      ...
      try {
        sig = Signature.fromBytes(ensureBytes3("sig", sg), "der");
      } catch (derError) {
      ...
      if (!sig) {
        try {
          sig = Signature.fromBytes(ensureBytes3("sig", sg), "compact");
        } catch (error) {
          return false;
        }
      }
  ...
  function verify(signature, message, publicKey, opts = {}) {
    ...
    message = validateMsgAndHash(ensureBytes3("message", message), prehash);
    if ("strict" in opts)
      throw new Error("options.strict was renamed to lowS");
    const sig = format2 === void 0 ? tryParsingSig(signature) : Signature.fromBytes(ensureBytes3("sig", signature), format2);
    if (sig === false)
      return false;
    try {
...
function createCurve3(curveDef, defHash) {
  const create2 = (hash3) => weierstrass3({ ...curveDef, hash: hash3 });
  return { ...create2(defHash), create: create2 };
}
...
var secp256k13 = createCurve3({ ...secp256k1_CURVE, Fp: Fpk13, lowS: true, endo: secp256k1_ENDO }, sha256);
...
var INFINITE_DRIVE = {
  ...
  chainId: 421018n,
  /** Designated testnet EVM chain ID. Override via env in deploy scripts. */
  testnetChainId: 421019n,
  rpcUrl: "https://infinitedrive.xyz",
  rpcUrlFallback: "https://evm-rpc.infinitedrive.xyz",
  explorerUrl: "https://scan.infinitedrive.xyz",
  nativeSymbol: "42",
  nativeDecimals: 18n,
  /** Remix-deployed, explorer-verified 2026-09-17. */
  announcer: "0xea4249A3777b36594230501b88528A28A98c9E86",
  registry: "0x2204311689350C1Eb63fF9FDE427ff05fb3bF476"
};
var SCHEME_ID_SECP256K1 = 1n;
var NATIVE_TOKEN_METADATA_ADDRESS = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
var NATIVE_TOKEN_FUNCTION_ID = 4008636142;
...
// frontend/eip712.ts
var STEALTH_EIP712_DOMAIN_NAME = "Infinite Drive Stealth";
var STEALTH_EIP712_DOMAIN_VERSION = "1";
var STEALTH_EIP712_TYPES = {
  StealthKeyDerivation: [
    { name: "purpose", type: "string" },
    { name: "account", type: "address" },
    { name: "chainId", type: "uint256" }
  ]
};
var STEALTH_EIP712_PURPOSE = "Derive stealth spend and view keys. Not a transaction. Does not move funds or grant token approval.";
function stealthKeyDerivationTypedData(account, chainId) {
  if (chainId <= 0n) throw new Error("chainId must be a positive bigint");
  if (chainId > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error("chainId exceeds EIP-712 domain numeric range");
  }
  return {
    domain: {
      name: STEALTH_EIP712_DOMAIN_NAME,
      version: STEALTH_EIP712_DOMAIN_VERSION,
      chainId: Number(chainId)
    },
    types: STEALTH_EIP712_TYPES,
    primaryType: "StealthKeyDerivation",
    message: {
      purpose: STEALTH_EIP712_PURPOSE,
      account,
      chainId
    }
  };
...
var COMPRESSED_PUBKEY_LEN = 33;
var UNCOMPRESSED_PUBKEY_LEN = 65;
var META_COMPRESSED_LEN = 66;
var META_UNCOMPRESSED_LEN = 130;
var HEX_RE = /^[0-9a-fA-F]*$/;
var META_PREFIX_RE = /^st:([a-z0-9]+):/i;
function assertNeverNumber(label, value) {
  if (typeof value === "number") {
    throw new Error(`${label} must be bigint, not number`);
  }
}
...
function fromHex6(hex) {
  ...
  return hexToBytes2(raw.toLowerCase());
}
function bytesToBigInt2(bytes) {
  if (bytes.length === 0) return 0n;
  return BigInt(toHex3(bytes));
}
function bigIntToBytes32(value) {
  assertNeverNumber("scalar", value);
  if (value < 0n) throw new Error("scalar must be non-negative");
  let hex = value.toString(16);
  if (hex.length > 64) throw new Error("scalar exceeds 32 bytes");
  hex = hex.padStart(64, "0");
  return fromHex6(hex);
}
function modN2(value) {
  const r = value % N;
  return r < 0n ? r + N : r;
}
...
function requireValidScalar(value, label) {
  const s = modN2(value);
  if (!isValidScalar(s)) {
...
function requireCsprng() {
  if (typeof globalThis.crypto?.getRandomValues !== "function") {
    throw new Error(
      "CSPRNG unavailable: this environment has no crypto.getRandomValues. Stealth keys must be generated in the browser (or Node 19+ / WebCrypto)."
    );
  }
}
...
function randomSecretKey() {
  requireCsprng();
  const utils = secp256k13.utils;
  if (typeof utils.randomPrivateKey === "function") {
    return utils.randomPrivateKey();
  }
  if (typeof utils.randomSecretKey === "function") {
    return utils.randomSecretKey();
  }
  for (; ; ) {
    const candidate = new Uint8Array(32);
    globalThis.crypto.getRandomValues(candidate);
    const k = bytesToBigInt2(candidate);
    if (isValidScalar(k)) return candidate;
  }
}
function getPoint(pub) {
  try {
    return secp256k13.ProjectivePoint.fromHex(bytesToHex2(pub));
  } catch (err) {
    throw new Error(`invalid secp256k1 public key: ${err.message}`);
  }
}
function ecdhUncompressed(privateKey, publicKey) {
  const priv = requireValidScalar(bytesToBigInt2(privateKey), "ECDH private key");
  if (typeof secp256k13.getSharedSecret === "function") {
    const shared = secp256k13.getSharedSecret(privateKey, publicKey, false);
    if (shared.length === UNCOMPRESSED_PUBKEY_LEN && shared[0] === 4) {
      return shared;
    }
    if (shared.length === 64) {
      const out = new Uint8Array(UNCOMPRESSED_PUBKEY_LEN);
      out[0] = 4;
      out.set(shared, 1);
      return out;
    }
    if (shared.length === COMPRESSED_PUBKEY_LEN) {
      return getPoint(shared).toRawBytes(false);
    }
  }
  const sharedPoint = getPoint(publicKey).multiply(priv);
  if (sharedPoint.equals(secp256k13.ProjectivePoint.ZERO)) {
    throw new Error("ECDH produced the point at infinity");
  }
  return sharedPoint.toRawBytes(false);
}
function hashSharedSecret(sharedUncompressed) {
  if (sharedUncompressed.length !== UNCOMPRESSED_PUBKEY_LEN || sharedUncompressed[0] !== 4) {
    throw new Error("shared secret must be a 65-byte uncompressed SEC1 point");
  }
  const hash3 = keccak_256(sharedUncompressed);
  const scalar = requireValidScalar(bytesToBigInt2(hash3), "hashed shared secret");
  return { hash: hash3, scalar, viewTag: hash3[0] };
}
function pubkeyToAddress(uncompressed) {
  const raw = uncompressed.length === UNCOMPRESSED_PUBKEY_LEN && uncompressed[0] === 4 ? uncompressed.subarray(1) : uncompressed.length === 64 ? uncompressed : (() => {
    throw new Error("pubkeyToAddress expects uncompressed secp256k1 key");
  })();
  const hash3 = keccak_256(raw);
  return toChecksumAddress(hash3.subarray(12));
}
function toChecksumAddress(addr20) {
  if (addr20.length !== 20) throw new Error("address must be 20 bytes");
  const lower = bytesToHex2(addr20);
  const hash3 = keccak_256(new TextEncoder().encode(lower));
  let out = "0x";
  for (let i = 0; i < 40; i++) {
    const nibble = parseInt(lower[i], 16);
    const hashNibble = hash3[i >> 1];
    const hashVal = i % 2 === 0 ? hashNibble >> 4 : hashNibble & 15;
    out += nibble > 9 && hashVal >= 8 ? lower[i].toUpperCase() : lower[i];
  }
  return out;
}
function parsePubKey(input) {
  const bytes = typeof input === "string" ? fromHex6(input) : input;
  if (bytes.length === COMPRESSED_PUBKEY_LEN) {
    const p = bytes[0];
    if (p !== 2 && p !== 3) throw new Error("compressed pubkey prefix must be 02 or 03");
    getPoint(bytes);
    return bytes;
  }
  if (bytes.length === UNCOMPRESSED_PUBKEY_LEN) {
    if (bytes[0] !== 4) throw new Error("uncompressed pubkey prefix must be 04");
    getPoint(bytes);
    return bytes;
  }
...
function compressPub(pub) {
  if (pub.length === COMPRESSED_PUBKEY_LEN) return pub;
  return getPoint(pub).toRawBytes(true);
}
function parseStealthMetaAddress(input) {
  let chainShortName = null;
  let bytes;
  if (typeof input === "string") {
    const trimmed = input.trim();
    const prefix = trimmed.match(META_PREFIX_RE);
    const body = prefix ? trimmed.slice(prefix[0].length) : trimmed;
    if (prefix) chainShortName = prefix[1].toLowerCase();
    bytes = fromHex6(body);
  } else {
    bytes = input;
  }
  if (bytes.length === META_COMPRESSED_LEN) {
    const spendPublicKey = parsePubKey(bytes.subarray(0, 33));
    const viewPublicKey = parsePubKey(bytes.subarray(33, 66));
    return { spendPublicKey, viewPublicKey, chainShortName };
  }
  if (bytes.length === META_UNCOMPRESSED_LEN) {
    const spendPublicKey = compressPub(parsePubKey(bytes.subarray(0, 65)));
    const viewPublicKey = compressPub(parsePubKey(bytes.subarray(65, 130)));
    return { spendPublicKey, viewPublicKey, chainShortName };
  }
  throw new Error(
    `stealth meta-address must be 66 (compressed) or 130 (uncompressed) bytes, got ${bytes.length}`
  );
}
function metaAddressToRegistryBytes(metaAddress) {
  const parsed = parseStealthMetaAddress(metaAddress);
  const concat3 = new Uint8Array(META_COMPRESSED_LEN);
  concat3.set(parsed.spendPublicKey, 0);
  concat3.set(parsed.viewPublicKey, 33);
  return toHex3(concat3);
}
function encodeViewTagMetadata(viewTag, extra) {
  if (!Number.isInteger(viewTag) || viewTag < 0 || viewTag > 255) {
    throw new Error("view tag must be an integer in [0, 255]");
  }
  const extraBytes = extra === void 0 ? new Uint8Array(0) : typeof extra === "string" ? fromHex6(extra) : extra;
  const out = new Uint8Array(1 + extraBytes.length);
  out[0] = viewTag;
  out.set(extraBytes, 1);
  return toHex3(out);
}
function encodeNativeMetadata(viewTag, amountWei) {
  assertNeverNumber("amountWei", amountWei);
  if (amountWei < 0n) throw new Error("amountWei must be >= 0");
  if (amountWei >= 1n << 256n) throw new Error("amountWei exceeds uint256");
  const extra = new Uint8Array(4 + 20 + 32);
  extra[0] = NATIVE_TOKEN_FUNCTION_ID >>> 24 & 255;
  extra[1] = NATIVE_TOKEN_FUNCTION_ID >>> 16 & 255;
  extra[2] = NATIVE_TOKEN_FUNCTION_ID >>> 8 & 255;
  extra[3] = NATIVE_TOKEN_FUNCTION_ID & 255;
  extra.set(fromHex6(NATIVE_TOKEN_METADATA_ADDRESS), 4);
  extra.set(bigIntToBytes32(amountWei), 24);
  return encodeViewTagMetadata(viewTag, extra);
}
function readViewTag(metadata) {
  const bytes = typeof metadata === "string" ? fromHex6(metadata) : metadata;
  return bytes.length === 0 ? null : bytes[0];
}
function deriveFromKeys(spendPub, viewPub, ephemeralPriv, extraMetadata) {
  const shared = ecdhUncompressed(ephemeralPriv, viewPub);
  const { scalar, viewTag } = hashSharedSecret(shared);
  const stealthPoint = getPoint(spendPub).add(secp256k13.ProjectivePoint.BASE.multiply(scalar));
  if (stealthPoint.equals(secp256k13.ProjectivePoint.ZERO)) {
    throw new Error("derived stealth public key is the point at infinity");
  }
  const stealthUncompressed = stealthPoint.toRawBytes(false);
  const stealthAddress = pubkeyToAddress(stealthUncompressed);
  const ephemeralPubKey = toHex3(secp256k13.getPublicKey(ephemeralPriv, true));
  return {
    stealthAddress,
    ephemeralPubKey,
    viewTag: toHex3(Uint8Array.of(viewTag)),
    metadata: encodeViewTagMetadata(
      viewTag,
      extraMetadata
    ),
...
function deriveStealthAddress(recipientMetaAddress, options = {}) {
  const parsed = parseStealthMetaAddress(recipientMetaAddress);
  const ephemeralPriv = randomSecretKey();
  try {
    return deriveFromKeys(
      parsed.spendPublicKey,
      parsed.viewPublicKey,
      ephemeralPriv,
      options.extraMetadata
    );
  } finally {
    zeroize(ephemeralPriv);
  }
}

// frontend/relayer-quote.ts
function quoteSweep(input) {
  const gasLimit = input.gasLimit ?? 21000n;
  const bufferBps = input.bufferBps ?? 2000n;
  const relayerTipWei = input.relayerTipWei ?? 0n;
  if (input.gasPriceWei < 0n || gasLimit <= 0n || bufferBps < 0n || relayerTipWei < 0n) {
    throw new Error("quote inputs must be non-negative bigints");
  }
  const gasCostWei = input.gasPriceWei * gasLimit;
  const buffered = gasCostWei + gasCostWei * bufferBps / 10000n;
  const reservedWei = buffered + relayerTipWei;
  const recommendedStipendWei = reservedWei + buffered;
  return { gasCostWei, reservedWei, relayerTipWei, recommendedStipendWei };
}
function maxSweepValue(balanceWei, reservedWei) {
  if (balanceWei < 0n || reservedWei < 0n) throw new Error("balances must be bigint >= 0");
  return balanceWei > reservedWei ? balanceWei - reservedWei : 0n;
}

// frontend/units.ts
function placeCount(decimals) {
  if (decimals < 0n || decimals > 78n) {
    throw new Error("decimals out of range");
  }
  return Number(decimals);
}
function parseAmountToWei(input, decimals = 18n) {
  const s = input.trim();
  if (!s) throw new Error("amount required");
  if (!/^\d+(\.\d+)?$/.test(s)) throw new Error("invalid amount");
  const places = placeCount(decimals);
  const [wholeRaw, fracRaw = ""] = s.split(".");
  if (fracRaw.length > places) throw new Error("too many decimal places");
  const frac = fracRaw.padEnd(places, "0");
  const combined = `${wholeRaw}${frac}`.replace(/^0+(?=\d)/, "");
  return BigInt(combined === "" ? "0" : combined);
}
function formatWei(wei, decimals = 18n) {
  if (typeof wei !== "bigint") throw new Error("wei must be bigint");
  const places = placeCount(decimals);
  const neg = wei < 0n;
  const v = neg ? -wei : wei;
  const base = 10n ** decimals;
  const whole = v / base;
  const frac = (v % base).toString().padStart(places, "0").replace(/0+$/, "");
  const s = frac.length === 0 ? whole.toString(10) : `${whole.toString(10)}.${frac}`;
  return neg ? `-${s}` : s;
}

// web/app.ts
var SETTINGS_KEY = "infinite-stealth-settings-v2";
var DEFAULT_RPC = INFINITE_DRIVE.rpcUrlFallback;
var DEFAULT_CHAIN = INFINITE_DRIVE.chainId;
var DEFAULT_GATEWAY = "http://127.0.0.1:4174";
var state = { account: null, walletChainId: null, identity: null };
var $ = (id) => {
...
var WorkerBridge = class {
  worker = new Worker("./stealth.worker.js", { type: "module" });
  next = 1;
  ...
  constructor() {
    this.worker.addEventListener("message", (ev) => {
      ...
      else job.reject(new Error(ev.data.error ?? "worker error"));
    });
  }
  call(payload) {
    const id = this.next++;
...
var ring = new WorkerBridge();
function loadSettings() {
  const fallback = {
    rpcUrl: DEFAULT_RPC,
    chainId: DEFAULT_CHAIN.toString(10),
    announcer: INFINITE_DRIVE.announcer,
    registry: INFINITE_DRIVE.registry,
    fromBlock: "0",
...
function settings() {
  return {
    rpcUrl: $("cfg-rpc").value.trim() || DEFAULT_RPC,
    chainId: $("cfg-chain").value.trim() || DEFAULT_CHAIN.toString(10),
    announcer: $("cfg-announcer").value.trim(),
    registry: $("cfg-registry").value.trim(),
    fromBlock: $("cfg-from").value.trim() || "0",
    gateway: $("cfg-gateway").value.trim() || DEFAULT_GATEWAY
  };
}
function saveSettings(s) {
  localStorage.setItem(SETTINGS_KEY, JSON.stringify(s));
}
function chainIdFromSettings() {
  const raw = settings().chainId;
  const id = raw.startsWith("0x") ? BigInt(raw) : BigInt(raw);
  if (id <= 0n) throw new Error("chain ID must be a positive bigint");
  return id;
}
function driveChain() {
  const id = chainIdFromSettings();
  if (id > BigInt(Number.MAX_SAFE_INTEGER)) throw new Error("chain ID exceeds safe display range");
  return defineChain({
    id: Number(id),
    name: INFINITE_DRIVE.name,
    nativeCurrency: { name: "Improbability", symbol: "42", decimals: 18 },
    rpcUrls: { default: { http: [settings().rpcUrl || DEFAULT_RPC] } },
    blockExplorers: { default: { name: "Infinite Scan", url: INFINITE_DRIVE.explorerUrl } }
  });
}
function publicClient() {
  return createPublicClient({ chain: driveChain(), transport: http(settings().rpcUrl || DEFAULT_RPC) });
}
function injected() {
  const eth = window.ethereum;
  if (!eth) throw new Error("No injected wallet. Install MetaMask or a compatible wallet.");
  return eth;
}
...
function requireAddress(label, value) {
  const v = value.trim();
  if (!/^0x[0-9a-fA-F]{40}$/.test(v)) throw new Error(`${label} must be a 0x address`);
  return v;
}
function fillSettingsForm() {
  const s = loadSettings();
  $("cfg-rpc").value = s.rpcUrl;
  $("cfg-chain").value = s.chainId;
  $("cfg-announcer").value = s.announcer;
  $("cfg-registry").value = s.registry;
  $("cfg-from").value = s.fromBlock;
  $("cfg-gateway").value = s.gateway;
  $("scan-from").value = s.fromBlock;
}
function paintNetwork() {
  const want = chainIdFromSettings();
  const got = state.walletChainId;
  $("net-pill").textContent = got ? got === want ? `Network: ${got.toString(10)}` : `Network: wallet ${got.toString(10)} \xB7 page ${want.toString(10)}` : `Network: disconnected \xB7 want ${want.toString(10)}`;
}
function paintIdentity() {
  $("meta-display").textContent = state.identity ? state.identity.stealthMetaAddress : "Locked. Sign typed data to unlock the worker.";
}
async function connect() {
  ...
  state.walletChainId = BigInt(await eth.request({ method: "eth_chainId" }));
  $("btn-connect").textContent = `${accounts[0].slice(0, 6)}\u2026${accounts[0].slice(-4)}`;
  $("btn-connect").classList.add("connected");
  paintNetwork();
}
async function switchToInfinite() {
  const eth = injected();
  const id = chainIdFromSettings();
  ...
  try {
    ...
    await eth.request({
      ...
      params: [
        {
          chainId: hex,
          chainName: INFINITE_DRIVE.name,
          nativeCurrency: { name: "Improbability", symbol: "42", decimals: 18 },
          rpcUrls: [settings().rpcUrl || DEFAULT_RPC, INFINITE_DRIVE.rpcUrl],
          blockExplorerUrls: [INFINITE_DRIVE.explorerUrl]
        }
      ]
    });
  }
...
async function deriveFromWallet() {
  ...
  const out = await ring.call({ type: "unlock", signature: sig });
  state.identity = out.identity;
  paintIdentity();
  setStatus("receive-status", "Worker unlocked. Private keys did not enter this page.", "ok");
}
async function forgetKeys() {
  await ring.call({ type: "forget" });
  state.identity = null;
  paintIdentity();
}
async function registerMeta() {
  ...
  const hash3 = await walletClient().writeContract({
    ...
    args: [metaAddressToRegistryBytes(state.identity.stealthMetaAddress)],
    chain: driveChain(),
    account: state.account
  });
  setStatus("receive-status", `Registered. tx ${hash3}`, "ok");
}
async function resolveRecipientMeta(input) {
  const t = input.trim();
  if (!t) throw new Error("Recipient required");
  if (t.startsWith("st:") || t.startsWith("0x") && t.length > 42) return t;
  const registry = requireAddress("registry", settings().registry);
  const bytes = await publicClient().readContract({
    address: registry,
    abi: registryAbi,
    functionName: "getStealthMetaAddress",
    args: [requireAddress("recipient", t)]
  });
  if (!bytes || bytes === "0x") throw new Error("That wallet has not registered a stealth meta-address");
  return bytes;
}
function payAmounts() {
  const amount = parseAmountToWei($("pay-amount").value || "0");
  const stipend = parseAmountToWei($("pay-stipend").value || "0");
  if (amount <= 0n) throw new Error("Amount must be > 0");
  if (stipend < 0n) throw new Error("Stipend must be >= 0");
  return { amount, stipend, total: amount + stipend };
}
function refreshPayTotals() {
  try {
    const { stipend, total } = payAmounts();
    $("pay-total").textContent = `${formatWei(total)} 42`;
    $("pay-quote").textContent = stipend === 0n ? "0 42 \u2014 recipient may be unable to self-sweep" : `${formatWei(stipend)} 42 stipend`;
  } catch {
    $("pay-total").textContent = "\u2014";
  }
}
async function refreshGasQuote() {
  try {
    const q = quoteSweep({ gasPriceWei: await publicClient().getGasPrice() });
    const stipendInput = $("pay-stipend");
    if (!stipendInput.value) stipendInput.value = formatWei(q.recommendedStipendWei);
    $("pay-quote").textContent = `${formatWei(q.recommendedStipendWei)} 42 recommended`;
    refreshPayTotals();
  } catch {
    $("pay-quote").textContent = "Could not quote gas \u2014 set RPC in Settings";
  }
}
async function pay() {
  await connect();
  ...
  const derived = deriveStealthAddress(meta);
  const viewTag = readViewTag(derived.metadata);
  if (viewTag === null) throw new Error("missing view tag");
  const includeAmount = $("pay-meta-amount").checked;
  const metadata = includeAmount ? encodeNativeMetadata(viewTag, amount) : derived.metadata;
  setStatus("pay-status", `Sending ${formatWei(total)} 42 to ${derived.stealthAddress}\u2026`);
  ...
  setStatus("pay-status", "Announcing (view tag only unless you opted into amount metadata)\u2026");
  const announceHash = await walletClient().writeContract({
    address: announcer,
    abi: announcerAbi,
    functionName: "announce",
    args: [SCHEME_ID_SECP256K1, derived.stealthAddress, derived.ephemeralPubKey, metadata],
    chain: driveChain(),
    account: state.account
  });
...
async function scan() {
  if (!state.identity) throw new Error("Unlock keys on Receive first");
  const logs = await loadAnnouncements();
  const out = await ring.call({ type: "scan", logs });
  const hitsEl = $("scan-hits");
  hitsEl.replaceChildren();
  for (const hit of out.hits) {
    const bal = await publicClient().getBalance({ address: hit.stealthAddress });
    const card = document.createElement("div");
    card.className = "hit";
    const head = document.createElement("header");
    const title = document.createElement("strong");
    title.className = "mono";
    title.textContent = hit.stealthAddress;
    const amt = document.createElement("span");
    amt.textContent = `${formatWei(bal)} 42`;
    head.append(title, amt);
    card.append(head);
    if (sweepEnabled()) {
      const btn = document.createElement("button");
      btn.className = "ghost";
      btn.type = "button";
      btn.textContent = "Sweep via worker + relayer/RPC";
      btn.addEventListener("click", () => {
        void (async () => {
          try {
            await sweep(hit);
          } catch (err) {
            setStatus("scan-status", err instanceof Error ? err.message : String(err), "err");
          }
        })();
      });
      card.append(btn);
  ...
  setStatus(
    "scan-status",
    out.hits.length === 0 ? `No matches in ${logs.length.toString(10)} logs (view-only).` : `Found ${out.hits.length.toString(10)} \xB7 sweep ${sweepEnabled() ? "enabled" : "disabled (view-only)"}`,
    out.hits.length === 0 ? "" : "ok"
  );
}
async function broadcastRaw(raw) {
  ...
  if (gateway) {
    try {
      const res = await fetch(`${gateway}/v1/broadcast`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ rawTransaction: raw })
      });
      const body = await res.json();
      if (res.ok && body.hash) return body.hash;
    } catch {
    }
  }
  return publicClient().sendRawTransaction({ serializedTransaction: raw });
}
async function sweep(hit) {
  if (!sweepEnabled()) throw new Error("Enable sweep first");
  const to = requireAddress("withdraw", $("scan-to").value);
  if (state.account && to.toLowerCase() === state.account.toLowerCase()) {
    throw new Error("Refusing to sweep to the connected wallet. Use a fresh address.");
  }
  const client = publicClient();
  const [balance, gasPrice, nonce] = await Promise.all([
    client.getBalance({ address: hit.stealthAddress }),
    client.getGasPrice(),
    client.getTransactionCount({ address: hit.stealthAddress })
  ]);
  const q = quoteSweep({ gasPriceWei: gasPrice });
  const value = maxSweepValue(balance, q.reservedWei);
  if (value === 0n) {
    throw new Error(
      `Balance ${formatWei(balance)} 42 is too low. Need about ${formatWei(q.recommendedStipendWei)} 42 stipend.`
    );
  }
  const signed = await ring.call({
    type: "signSweep",
    input: {
      ephemeralPubKey: hit.ephemeralPubKey,
      expectedStealthAddress: hit.stealthAddress,
      to,
      value,
      nonce,
      gas: 21000n,
      gasPrice,
      chainId: chainIdFromSettings()
    }
  });
  const hash3 = await broadcastRaw(signed.rawTransaction);
  setStatus("scan-status", `Sweep broadcast ${hash3} \xB7 ${formatWei(value)} 42 to ${to}`, "ok");
}
function bindTabs() {
...
function onErr(id, fn) {
  return () => {
    void (async () => {
      try {
        await fn();
      } catch (err) {
        setStatus(id, err instanceof Error ? err.message : String(err), "err");
      }
    })();
  };
}
function init() {
  ...
  paintNetwork();
  paintIdentity();
  $("btn-connect").addEventListener("click", onErr("receive-status", connect));
  $("btn-switch").addEventListener("click", onErr("receive-status", switchToInfinite));
  $("btn-derive").addEventListener("click", onErr("receive-status", deriveFromWallet));
  $("btn-register").addEventListener("click", onErr("receive-status", registerMeta));
  $("btn-copy-meta").addEventListener("click", onErr("receive-status", async () => {
    if (!state.identity) throw new Error("Unlock first");
    await navigator.clipboard.writeText(state.identity.stealthMetaAddress);
    setStatus("receive-status", "Public meta-address copied.", "ok");
  }));
  $("btn-forget").addEventListener("click", onErr("receive-status", async () => {
    await forgetKeys();
    setStatus("receive-status", "Worker locked.", "ok");
  }));
  $("btn-pay").addEventListener("click", onErr("pay-status", pay));
  $("btn-scan").addEventListener("click", onErr("scan-status", scan));
  $("btn-save-cfg").addEventListener("click", () => {
    saveSettings(settings());
    setStatus("cfg-status", "Saved. Keys were not saved.", "ok");
    paintNetwork();
    void refreshGasQuote();
  });
  $("pay-amount").addEventListener("input", refreshPayTotals);
  $("pay-stipend").addEventListener("input", refreshPayTotals);
  window.addEventListener("beforeunload", () => {
    void ring.call({ type: "forget" });
    ring.terminate();
  });
  void refreshGasQuote();
}
